...
@implementation CCKVStorageItem

@end

@implementation CCKVStorage
{
...
- (CCKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    CCKVStorageItem *item = [CCKVStorageItem new];
...
- (CCKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    CCKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            CCKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    ...
    return [self initWithPath:nil type:CCKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(CCKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"YYKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > CCKVStorageTypeMixed) {
    ...
    _trashQueue = dispatch_queue_create("com.cxk.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
...
- (BOOL)saveItemWithkey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (filename.length) {
        ...
        if (_type != CCKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case CCKVStorageTypeSQLite:
            return [self _dbDeleteItemWithKey:key];
            break;
            case CCKVStorageTypeFile:
        case CCKVStorageTypeMixed: {
            NSString * filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (CCKVStorageItem * item in items) {
            if (total > maxSize ) {
                if (item.filename) {
...
- (CCKVStorageItem *)getItemForkey:(NSString *)key {
    ...
    CCKVStorageItem * item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (CCKVStorageItem *)getitemInfoForKey:(NSString *)key {
    ...
    CCKVStorageItem * item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
